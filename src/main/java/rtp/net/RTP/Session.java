...
import rtp.net.InetAddressFactory;
import sun.net.*;
...
public class Session extends java.lang.Object {
     ...
     */
    public RTPThreadHandler m_RTPHandler = null;


    /**
     * Reference to the RTCP Sender and Receiver Thread handler.
     */
    public RTCPThreadHandler m_RTCPHandler = null;
     ...
     */
    private LateInitializationServer LateInitSrvr;


    /**
     * Reference to the Late Initialization Client.
     */
    private LateInitializationClient LateInitClient;
     ...
     */
    public Vector InitializationData = new Vector();
    ...
    public int UpdateSources() {
        ...
        while (SourceCollection.hasMoreElements()) {
            ...
            if ((s.TimeOfLastRTPArrival < CurrentTime() - T * 1000) && s.ActiveSender == true) {
                s.ActiveSender = false;
                outprintln("Degrading from a sender to member");
                outprintln("SSRC = 0x" + Integer.toHexString((int) s.SSRC));
                n++;
            }
            ...
            if ((s.TimeOfLastRTCPArrival < (CurrentTime() - 5 * Td * 1000)) && s.SSRC != SSRC) {
                //outprintln();
                //outprintln("Td" + Td);
                //outprintln("Removing Source : " + "SSRC = 0x" + Integer.toHexString((int)s.SSRC));
                outprintln("Removing Source : " + "SSRC = 0x" + Integer.toHexString((int) s.SSRC));
                //outprintln("Current Time "  + Session.CurrentTime());
                //outprintln ("s.TimeOfLastRTCPArrival" + s.TimeOfLastRTCPArrival);
                RemoveSource(s.SSRC);
    ...
    public Session(String MulticastGroupIPAddress,
        ...
        m_RTPHandler = new RTPThreadHandler(m_InetAddress, RTPSendFromPort, MulticastGroupPort, this);

        // Create a new RTCP Handler thread (but do not start it yet)
        //  Set the sendto and recvfrom ports
        m_RTCPHandler = new RTCPThreadHandler(m_InetAddress, RTCPSendFromPort, RTCPGroupPort, this);
     ...
     */
    public synchronized void StartRTCPReceiverThread() {
     ...
     */
    public synchronized void StartRTCPSenderThread() {
    ...
    public synchronized int RemoveSource(long sourceSSRC) {
        if (SourceMap.get(new Long(sourceSSRC)) != null) {
            ...
            outprintln("Removing Source : " + "SSRC = 0x" + Integer.toHexString((int) sourceSSRC));
            outprintln("No. of members" + GetNumberOfMembers());
            outprintln("No. of senders" + GetNumberOfActiveSenders());
        } else {
    ...
    /**
     * Calculates the next interval, sets the T and Td class level  variables.
     * <p>
    ...
    public int Initialize() {
        ...
        AddSource(SSRC, new Source(SSRC));
    ...
    public int AddSource(long newSSRC, Source src) {
        ...
        if (SourceMap.containsKey(new Long(newSSRC))) {
            ...
            outprintln("Adding Source : " + "SSRC = 0x" + Integer.toHexString((int) newSSRC));
            outprintln("No. of members" + GetNumberOfMembers());
            outprintln("No. of senders" + GetNumberOfActiveSenders());
     ...
     */
    public synchronized void addRTCP_actionListener(RTCP_actionListener listener) {
     ...
     */
    public synchronized void addRTP_actionListener(RTP_actionListener listener) {
     ...
     */
    public synchronized void StartLateInitializationDeamon() {
        if (DebugOutput) LateInitSrvr.StartServerDeamon(9606, this);
    }
    ...
    public synchronized void StartLateInitializationClient
    (String TCPServerAddress,
     int ServicePort
    ) {
        LateInitClient.StartClient(TCPServerAddress, ServicePort, this);
    }
