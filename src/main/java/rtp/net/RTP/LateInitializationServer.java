...
public class LateInitializationServer extends java.lang.Object
{
    ...
    */
    ...
    public int StartServerDeamon (int portToMonitor, Session session )
    {
		InitilizationServerThread InitSrvrThread = new InitilizationServerThread(portToMonitor, session);
		System.out.println("Server Started");
		System.out.println("Listening to port:  " + portToMonitor);
...
class InitilizationServerThread extends Thread
{
	...
	*/
	InitilizationServerThread(int portToMonitor, Session session)
	{
		...
		try
		{
			...
			System.out.println("Could not create server socket on port: " + port  + ", " + e.toString());
			System.exit(1);
		}
	}
	...
	public void run()
	{
		...
		while (true)
		{
			...
			try {
				...
				System.out.println("Main Server thread started: Waiting for Client Connections");
				System.out.println("New Client Connection Requested");
					    
			     
			    // Assign the client to listen to a randomly generated port
			    int NextSocket = GetNextPort();
    			ServerSocketThread SocketThread = new ServerSocketThread(NextSocket, session);
    			SocketThread.start();
    		
                dataInputStream = new DataInputStream(new BufferedInputStream(clientSocket.getInputStream()));
                printStream = new PrintStream(new BufferedOutputStream(clientSocket.getOutputStream(), 1024), false);

                System.out.println("Sending new Port Number to Client: " + NextSocket);
                printStream.println(NextSocket);
                printStream.flush();
				
				
			} catch (IOException e) {
				System.out.println("ftpThread.run(): accept failed on port " + port + ", " + e.toString());
				System.exit(1);
			}
...
class ServerSocketThread extends Thread
{
	...
	*/    
	ServerSocketThread(int portToMonitor, Session session)
	{
	...
	*/
	...
	public void run()
	{
		...
		while (true)
		{
			...
			try {
				clientSocket = serverSocket.accept();
				System.out.println("Server thread started on a new Socket: Waiting for Client Connections");
				System.out.println("New Client Connection Requested");
				ClientHandler clientHandler = new ClientHandler(this, clientSocket, session);
				clientHandler.start();
			} catch (IOException e) {
				System.out.println("ServerSocketThread.run(): accept failed on port " + port + ", " + e.toString());
				System.exit(1);
			}
...
class ClientHandler extends Thread
{
	...
	*/
	...
	ClientHandler(ServerSocketThread srvrSockThread, Socket clntSocket, Session session)
	{
		...
		try 
		{
			...
			*/
			...
			while (!done)
			{
				...
				System.out.println("Reading a line from client" + lineFromClient +" Socket" + clientSocket);
				...
				{
				    System.out.println("LOOP :Got a BYE"+" Socket" + clientSocket);
				    done =true;
				//	break;
				}
			}
			...
			System.out.println("ServerSocketThreadServer SOCKET:PROBLEM " + e.toString() + " SOCKET" + clientSocket);
		} finally {
			try 
			{
				if (printStream != null)
				...
				{
					clientSocket.close();
					System.out.println("Closing Port" + clientSocket);
				}
			} catch (IOException e) {
	...
	*/
	...
	private String processInput(String in)
	{
        ...
        */
	            ...
	            Enumeration elements = session.InitializationData.elements();
