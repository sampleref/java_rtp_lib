...
public final class LateInitializationClient extends java.lang.Object {
     ...
     */
    public void StartClient(String TCPServerAddress, int ServicePort, Session session) {
        try {
            try {
        ...
        new InitilizationClient(machine, ServicePort, session);
...
class InitilizationClient extends java.lang.Object {
    ...
    InitilizationClient(InetAddress machine, int ServicePort, Session session) {
        /**
         *   Create a thread to connect to the server
         */
        InitilizationGetPortThread InitGetPortThread = new InitilizationGetPortThread(machine, ServicePort, session);
        System.out.println("Client Started: on port" + portToMonitor);
...
class InitilizationGetPortThread extends Thread {
     ...
     */
    InitilizationGetPortThread(InetAddress machine, int portToMonitor, Session session) {
        ...
        try {
            ...
            System.out.println("Client Socket Created Succesfully at port" + portToMonitor);
    ...
    public void run() {
        ...
        try {
            ...
            System.out.println("Client:: Initial Thread Running");

            lineFromServer = dataInputStream.readLine();

            InitilizationClientThread InitClient = new InitilizationClientThread(host, Integer.valueOf(lineFromServer).intValue(), session);
            System.out.println("Client Started: on new port " + Integer.valueOf(lineFromServer).intValue());
            InitClient.start();
...
class InitilizationClientThread extends Thread {
    ...
    InitilizationClientThread(InetAddress machine, int portToMonitor, Session session) {
        super("InitilizationClientThread");
        //fileName = fName;
        host = machine;
        port = portToMonitor;
        ...
        try {
            ...
            System.out.println("Client Socket Created Succesfully");
            dataInputStream = new DataInputStream(clientSocket.getInputStream());
            printStream = new PrintStream(clientSocket.getOutputStream());
            Data = new Vector();
        ...
        switch (state) {
            case ESTABLISH_CONNECTION:
                ...
                if (in.equalsIgnoreCase("CONNECTION ESTABLISHED")) {
                    System.out.println("Entering ESTABLISH CONNECTION");
                    printStream.println("INITIALIZE");
                    printStream.flush();
                    state = INITIALIZE;
                    System.out.println("Rcvd: ESTABLISH CONNECTION from Server - Sending INITIALIZE");
            ...
            case INITIALIZE:
                ...
                if (in.equalsIgnoreCase("EOT")) {
                    ...
                    System.out.println("Rcvd: EOT from Server");
                } else {
            ...
            case END_OF_TRANSMISSION:
                ...
                System.out.println("noCharRecvd" + noCharRecvd + "noCharXfer" + noCharXfer);
                if (noCharRecvd == noCharXfer) {
                    printStream.println("OK");
                    System.out.println("Rcvd: Correct No. of Characters");
                    printStream.flush();
                    state = CLOSE_CONNECTION;
                } else {
                    printStream.println("NOK");
                    printStream.flush();
                    System.out.println("Rcvd: Wrong No. of Characters");
                    state = CLOSE_CONNECTION;
                }
